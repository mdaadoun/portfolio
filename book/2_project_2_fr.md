...
* **Options d'export :** L'utilisateur peut choisir le format via l'option `--output` / `-o` :
...
* **Qualité d'Ingénierie & Robustesse :**
  - [ ] Le code passe le linting Ruff et le contrôle de type strict Mypy sans aucune erreur.
  - [ ] Les exceptions d'API et erreurs réseau sont interceptées sans crash non géré.
* **Validation Fonctionnelle :**
  - [ ] Les 3 modes de saisie (URL, fichier local, texte direct) génèrent un rapport d'analyse conforme.
  - [ ] L'affichage console avec `Rich` restitue correctement les panneaux, métriques et tableaux.
  - [ ] L'export sous format Markdown ou JSON crée un fichier valide sur disque.
